...
// ---------------------------
// DYNAMIC SEO (WHATSAPP + X + FACEBOOK READY)
// ---------------------------
...
export default async function BlogPostPage({ params }: Props) {
  ...
  // ---------------------------
  // RELATED POSTS (SAFE QUERY)
  // ---------------------------
  let relatedPosts: any[] = [];

  try {
    const relatedQuery = await adminDb
  ...
  if (post.publishedAt) {
    if (typeof post.publishedAt.toDate === "function") {
      ...
      dateStr = new Date(post.publishedAt._seconds * 1000).toLocaleDateString(
        "en-US",
        {
          year: "numeric",
          month: "long",
          ...
          {post.tags && post.tags.length > 0 && (
            <div className="kb-tags-list">
              {post.tags.map((tag: string, i: number) => (
                <span key={i} className="kb-tag-item">
                  #{tag}
                </span>
              ))}
            </div>
