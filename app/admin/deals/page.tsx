...
export default function AdminDealsPage() {
  ...
  useEffect(() => {
    if (activeDeals.length > 0 && !campaignLocked && isStateLoaded) {
  ...
  }, [campaignType, activeDeals, campaignLocked, isStateLoaded]);

  // Handle Finding a Product to put on sale
  const handleSearch = async () => {
    if (!searchId.trim()) return alert("Please enter a Product ID");
    
    try {
      const q = query(collection(db, "products"), where("publicId", "==", searchId.trim()));
      const snap = await getDocs(q);
      if (!snap.empty) {
  ...
  const handleStartDeal = async () => {
    ...
    try {
      ...
      await updateDoc(docRef, {
        originalPrice: Number(selectedProduct.price), // Backup original price
        price: Number(salePrice),                     // Overwrite price for seamless Firebase filtering
        isSale: true,
        campaignType: campaignType,
        saleEndDate: new Date(endDate).toISOString(), // Master timer string
      });
  ...
  const handleEndDeal = async (product: any) => {
    if (!window.confirm(`Are you sure you want to end the deal for ${product.name || 'this item'} early?`)) return;
  ...
  if (!isStateLoaded) return <div className="p-12 text-center font-bold text-slate-500 animate-pulse">Loading dashboard...</div>;

  return (
    <div className="p-6 max-w-6xl mx-auto font-sans selection:bg-[#FF6A00] selection:text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-10">
          ...
          <div className="flex flex-col gap-5">
            ...
            {!campaignLocked ? (
              <button 
                ...
                className="bg-slate-900 text-white font-black uppercase tracking-widest py-3 rounded-md mt-2 hover:bg-slate-800 transition-colors flex items-center justify-center gap-2 shadow-sm active:scale-[0.98]"
              >
              ...
              <button 
                ...
                className="bg-slate-50 text-slate-600 border border-slate-200 font-black uppercase tracking-widest py-3 rounded-md mt-2 hover:bg-slate-100 transition-colors flex items-center justify-center gap-2 active:scale-[0.98]"
              >
        ...
        <div className={`lg:col-span-8 bg-white border border-slate-200 rounded-xl p-6 shadow-sm transition-opacity duration-300 ${!campaignLocked ? 'opacity-50 pointer-events-none grayscale-[50%]' : 'opacity-100'}`}>
          ...
          <div className="flex gap-2 mb-6">
            ...
            />
            <button onClick={handleSearch} className="bg-slate-900 text-white px-8 py-3 rounded-md font-bold hover:bg-slate-800 transition-colors shadow-sm">
              Search
            </button>
          </div>

          {selectedProduct && (
            <div className="bg-orange-50 border border-[#FF6A00]/20 rounded-lg p-5 flex flex-col sm:flex-row gap-6 items-center animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="flex items-end gap-3 w-full sm:w-auto">
                <div className="flex-1 sm:w-48">
                  ...
                  <input 
                    ...
                    className="w-full border border-[#FF6A00]/30 rounded-md px-4 py-2.5 outline-none focus:border-[#FF6A00] font-black text-slate-900 focus:ring-2 focus:ring-[#FF6A00]/20 transition-all"
                ...
                <button 
                  ...
                  className="bg-[#FF6A00] text-white font-black uppercase tracking-widest px-6 py-2.5 h-[42px] rounded-md hover:bg-[#e65f00] transition-colors shadow-sm flex items-center gap-2 shrink-0 active:scale-[0.96]"
                >
