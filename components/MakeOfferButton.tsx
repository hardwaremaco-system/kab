...
export default function MakeOfferButton({ product }: { product: any }) {
  ...
  const handleSendOffer = (e: React.FormEvent) => {
    ...
    const message = `Hello! I'm looking at your *${safeName}* listed for UGX ${currentPrice.toLocaleString()} on Mbarara Online.\n\nI have cash ready today, but my budget is *UGX ${numericOffer.toLocaleString()}*. Deal?\n\nLink: ${itemUrl}`;
  ...
  return (
    <>
      {/* THE TRIGGER BUTTON (Now styled to fit perfectly inside the "More Actions" drawer) */}
      <button 
        onClick={() => setIsOpen(true)}
        className="w-full bg-white text-slate-700 border border-slate-200 py-3 rounded-xl font-bold text-sm flex justify-center items-center gap-2"
      >
        <span>🤝</span> Make an Offer
      </button>

      {/* THE MODAL (Animations and soft focus rings removed for a snappy feel) */}
      {isOpen && (
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="bg-slate-50 border-b border-slate-100 p-6 flex justify-between items-center">
              ...
              <button 
                ...
                className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-slate-500 hover:text-slate-900 shadow-sm border border-slate-200"
              >
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleSendOffer} className="p-6">
              ...
              <div className="mb-8">
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-14 pr-4 py-4 rounded-xl border-2 border-slate-200 focus:border-[#D97706] outline-none text-lg font-bold text-slate-900"
                  />
                </div>
              ...
              <button 
                type="submit"
                className="w-full bg-[#25D366] text-white py-4 rounded-xl font-black text-lg hover:bg-[#20bd5a] flex justify-center items-center gap-2"
              >
